...
- Quiz-funnel analytics events (`quiz_started`, `topic_importance_completed`, `demographics_submitted`,
  `results_viewed`, `mnemonic_saved`/`mnemonic_restored`, `language_changed`, `quiz_restarted`), with
  `election` and `region_id` on every event, and error-boundary errors reported to analytics.
